...
def joinVG(obj1, tag1, obj2, tag2, blend, delFace, collapse):
    # Get vertex group
    vg1 = reorder_vertex(getVG(obj1, tag1), obj1)
    vg2 = reorder_vertex(getVG(obj2, tag2), obj2)
    ...
    if collapse:
        ...
        if blend:
            textures.storeUV(obj1, face1, obj2, face2, tag1+tag2, pairs) # TO FIX

        if delFace:
            delete_face(face1, obj1)
            delete_face(face2, obj2)
...
def delete_face(face, obj):
    ...
    bmesh_face = bm.faces[face.index]
