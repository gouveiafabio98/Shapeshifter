...
# ---- FUNCTIONS ----
def constructor(self, assets_evaluated, simmetry, mutation, blend, delFace, collapse):
    global used_assets
    ...
    tag1 = "front_body,center"
    obj1 = randomAsset(self, assets_evaluated.copy(), tag1)
    if obj1 == None:
        return
    obj1 = addObject(obj1)
    ...
    vgCombine(self, obj1, tag1, assets_evaluated, None, obj1)

def vgCombine(self, obj1, tag1, list, usedTag, obj2):
    global asset_blend
    global asset_delFace
    global asset_collapse
    ...
    for vg in obj1.vertex_groups:
        ...
        if tag2 != usedTag:
            ...
            obj2 = randomAsset(self, list.copy(), tag2)
            if obj2 == None:
                return
            ...
            combination.joinVG(self, obj1, tag2, obj2, new_tag, asset_blend, asset_delFace, asset_collapse)
            vgCombine(self, obj2, tag2, list, new_tag, obj1)
        else:
            nodeGroups.append({
                "name": usedTag,
...
def randomAsset(self, assets, vg):
    global used_assets
    global asset_simmetry
    global asset_mutation
    ...
    if asset == None:
        asset = random_weight(self, assets, vg)
    # Store the used asset in the list
    used_assets.append(asset)
    # Return the new asset
    return asset

def random_weight(self, assets, vg):
    if len(assets)==0:
        self.report({'ERROR'}, f'Asset With Vertex Group {vg} Not Found')
        return
    values = []
    weights = []
    for asset in assets:
