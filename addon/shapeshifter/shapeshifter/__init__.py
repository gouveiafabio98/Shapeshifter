...
class ButtonSelectionCut(Operator):
    ...
    def execute(self, context):
        ...
        loopcut.selection_cut(self, selectvgName.vg_name, unselectvgName.vg_name)
        return {"FINISHED"}
...
class CutEnumAssetType(PropertyGroup):
...
class CutEnumAssetSymmetry(PropertyGroup):
...
class CutPanel(Panel):
    ...
    def draw(self, context):
        layout = self.layout
        scene = context.scene
        assetTypes = scene.CutEnumAssetType
        # Type Identifier
        layout.row().separator()
        draw_title_divider(layout, "Asset Information", "MOD_FLUID")
        row = layout.row()
...
def register():
    ...
    bpy.types.Scene.CutEnumAssetType = bpy.props.PointerProperty(type=CutEnumAssetType)
    bpy.types.Scene.CutEnumAssetSymmetry = bpy.props.PointerProperty(type=CutEnumAssetSymmetry)
...
def unregister():
    ...
    del bpy.types.Scene.CutEnumAssetType
    del bpy.types.Scene.CutEnumAssetSymmetry
