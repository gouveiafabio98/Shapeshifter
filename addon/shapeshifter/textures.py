...
def storeUV(obj1, face1, obj2, face2, tag, pairs):
    obj1.data.update()
    obj2.data.update()
    # face1 and face2 store the face that shares all the other faces edges (contains the vg)
    ...
    # UV
    active_uv1 = bm1.loops.layers.uv.active
    active_uv2 = bm2.loops.layers.uv.active

    new_uv1 = bm1.loops.layers.uv.new(tag)
    new_uv2 = bm2.loops.layers.uv.new(tag)

    for assoc in association:
        for f1_loop in bm1.faces[assoc['f1'].index].loops:
            i1 = f1_loop.vert.index
            for f2_loop in bm2.faces[assoc['f2'].index].loops:
                i2 = f2_loop.vert.index
                if [i2, i1] in assoc['pairs']:
                    f1_loop[new_uv1].uv = f2_loop[active_uv2].uv
                    f2_loop[new_uv2].uv = f1_loop[active_uv1].uv
